...
public class FlowableButtonServiceImpl extends ServiceImpl<FlowableButtonMapper, FlowableButton> implements FlowableButtonService {
    ...
    @Override
    public void copyLastVersionButtonConfig(String lastVersionProcessDefinitionId, String newVersionProcessDefinitionId) {
        List<FlowableButton> flowableButtonList = this.list(new LambdaQueryWrapper<FlowableButton>()
                .eq(FlowableButton::getProcessDefinitionId, lastVersionProcessDefinitionId));
        if(ObjectUtil.isNotEmpty(flowableButtonList)) {
            flowableButtonList.forEach(flowableButton -> {
                //移除主键id
                flowableButton.setId(null);
                //设置为最新版定义id
                flowableButton.setProcessDefinitionId(newVersionProcessDefinitionId);
                //新增一条
                this.save(flowableButton);
            });
        }

...
public class FlowableOptionServiceImpl extends ServiceImpl<FlowableOptionMapper, FlowableOption> implements FlowableOptionService {
    ...
    public void addByProcessDefinition(ProcessDefinition processDefinition) {
        ...
        flowableOption.setTitle(DEFAULT_FLW_TITLE_REG);
    ...
    @Override
    public void copyLastVersionOptionConfig(String lastVersionProcessDefinitionId, String newVersionProcessDefinitionId) {
        FlowableOption flowableOption = this.getOne(new LambdaQueryWrapper<FlowableOption>()
                .eq(FlowableOption::getProcessDefinitionId, lastVersionProcessDefinitionId));
        if(ObjectUtil.isNotEmpty(flowableOption)) {
            //移除主键id
            flowableOption.setId(null);
            //设置为最新版定义id
            flowableOption.setProcessDefinitionId(newVersionProcessDefinitionId);
            //新增一条
            this.save(flowableOption);
        }
    }

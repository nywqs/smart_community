...
public class FlowableFormServiceImpl extends ServiceImpl<FlowableFormMapper, FlowableForm> implements FlowableFormService {
    ...
    @Override
    public void copyLastVersionFormConfig(String lastVersionProcessDefinitionId, String newVersionProcessDefinitionId) {
        List<FlowableForm> flowableFormList = this.list(new LambdaQueryWrapper<FlowableForm>()
                .eq(FlowableForm::getProcessDefinitionId, lastVersionProcessDefinitionId));
        if(ObjectUtil.isNotEmpty(flowableFormList)) {
            flowableFormList.forEach(flowableForm -> {
                //移除主键id
                flowableForm.setId(null);
                //设置为最新版定义id
                flowableForm.setProcessDefinitionId(newVersionProcessDefinitionId);
                //新增一条
                this.save(flowableForm);
            });
        }

...
public class FlowableEventServiceImpl extends ServiceImpl<FlowableEventMapper, FlowableEvent> implements FlowableEventService {
    ...
    @Override
    public void copyLastVersionEventConfig(String lastVersionProcessDefinitionId, String newVersionProcessDefinitionId) {
        List<FlowableEvent> flowableEventList = this.list(new LambdaQueryWrapper<FlowableEvent>()
                .eq(FlowableEvent::getProcessDefinitionId, lastVersionProcessDefinitionId));
        if(ObjectUtil.isNotEmpty(flowableEventList)) {
            flowableEventList.forEach(flowableEvent -> {
                //移除主键id
                flowableEvent.setId(null);
                //设置为最新版定义id
                flowableEvent.setProcessDefinitionId(newVersionProcessDefinitionId);
                //新增一条
                this.save(flowableEvent);
            });
        }

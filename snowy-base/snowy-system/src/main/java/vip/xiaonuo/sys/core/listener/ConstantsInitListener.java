...
public class ConstantsInitListener implements ApplicationListener<ApplicationContextInitializedEvent>, Ordered {
    ...
    private static final String FLOWABLE_ACT_TABLE_QUERY_SQL = "select * from act_de_model";
    ...
    public void onApplicationEvent(ApplicationContextInitializedEvent applicationContextInitializedEvent) {
        ...
        try{
            // 准备flowable工作流模块-是否自动初始化建表标志
            SqlExecutor.query(conn, FLOWABLE_ACT_TABLE_QUERY_SQL, new EntityListHandler());
            ConstantContext.putConstant(CommonConstant.FLOWABLE_DB_INITIALIZED_FLAG_NAME, true);
            log.info(">>> flowable模块数据库完成初始化标记: true");
        } catch (Exception e) {
            ConstantContext.putConstant(CommonConstant.FLOWABLE_DB_INITIALIZED_FLAG_NAME, false);
            log.info(">>> flowable模块数据库完成初始化标记: false");
        } finally {
            DbUtil.close(conn);
        }
    }

...
<script>
  ...
  import { merchantEdit } from '@/api/modular/main/merchant/merchantManage'
  export default {
    data () {
      return {
        labelCol: {
    ...
    methods: {
      ...
      edit (record) {
        ...
        // 时间单独处理
        if (record.openDate != null) {
            this.form.getFieldDecorator('openDate', { initialValue: moment(record.openDate, 'YYYY-MM-DD') })
        }
        this.openDateDateString = moment(record.openDate).format('YYYY-MM-DD')
      },
      handleSubmit () {

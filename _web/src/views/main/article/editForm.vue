...
<script>
  ...
  import { articleEdit } from '@/api/modular/main/article/articleManage'
  ...
  export default {
    ...
    methods: {
      ...
      edit (record) {
        ...
        // 时间单独处理
        if (record.publicTime != null) {
            this.form.getFieldDecorator('publicTime', { initialValue: moment(record.publicTime, 'YYYY-MM-DD') })
        }
        this.publicTimeDateString = moment(record.publicTime).format('YYYY-MM-DD')
        // 时间单独处理
        if (record.cancelTime != null) {
            this.form.getFieldDecorator('cancelTime', { initialValue: moment(record.cancelTime, 'YYYY-MM-DD') })
        }
      },
      handleSubmit () {
